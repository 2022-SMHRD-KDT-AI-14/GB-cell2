...
public class insertStateConI extends HttpServlet {
	...
	System.out.println("이상해");
		  ...
		  request.getSession(); String loginMember =
		  (String)session.getAttribute("loginMember"); String acc_link =
		  request.getParameter("acc_link"); String acc_time =
		  request.getParameter("arb_site"); int acc_pay= Integer.parseInt(
		  request.getParameter("arb_pay")); String cat_name =
		  request.getParameter("cat_name"); int board_seq =
		  Integer.parseInt(request.getParameter("board_seq"));
